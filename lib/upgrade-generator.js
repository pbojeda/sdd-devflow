...
function generateUpgrade(config) {
  ...
  for (const { posix, abs } of workflowCorePosixPaths) {
    ...
    // backup against the adapted template target in memory.
    const adaptedTarget = applyStackAdaptationsToContent(freshContent, posix, scan, config);
    if (normalizedContentEquals(backup, adaptedTarget)) {
  ...
  for (const spec of standardsSpecs) {
    ...
    // Case 3: fallback content-compare (no stored hash).
    if (normalizedContentEquals(existing, freshAdapted)) {
      backupBeforeReplace(dest, relativePath, backupTimestamp);
      fs.writeFileSync(absPath, freshAdapted, 'utf8');
      filesToAdapt.add(spec.posix);
      standardsResults.push({ name: spec.posix, modified: false });
