...
function generateUpgrade(config) {
  ...
  for (const { posix, abs } of workflowCorePosixPaths) {
    ...
    if (storedHash) {
      ...
      const adaptedTarget = applyStackAdaptationsToContent(freshContent, posix, scan, config);
      backupBeforeReplace(dest, relativePath, backupTimestamp);
    ...
    // backup against the adapted template target OR the raw template content.
    // Both are valid "pristine" states for a pre-v0.17.1 project:
    //   (a) adapted: --init ran applyStackAdaptations at install time
    //   (b) raw: generator.js scaffold copied template without running adapters
    // See the standards block fallback for the same rationale.
    const adaptedTarget = applyStackAdaptationsToContent(freshContent, posix, scan, config);
    if (
  ...
  const standardsSpecs = [
    {
      ...
      adapter: (tpl) => adaptBaseStandards(tpl, scan, config),
      include: true,
    },
    {
