...
5. ~~**No provenance tracking — cross-version pristine upgrades trigger false-positive preserve warnings (v0.16.10 accepted trade-off, v0.17.0 will fix)**~~ **RESOLVED in v0.17.0**. Content-addressable provenance via `.sdd-meta.json` (SHA-256 hashes keyed by POSIX path) now distinguishes "user edited since last tool-write" from "pristine since last tool-write" precisely. The hash path runs when `.sdd-meta.json` exists; a fallback content-compare path (with stack adaptations applied in-memory to the comparison target) handles the first v0.17.0 upgrade from a pre-v0.17.0 project. Preserved files deliberately keep their old hash (Codex M1 invariant) so a user-customized file that drifts and then gets reverted to canonical content is correctly re-recognized as pristine. See `dev/v0.17.0-provenance-plan.md` for the full design + cross-model review trail.
