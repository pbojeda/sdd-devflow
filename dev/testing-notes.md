...
| `template/gitignore` | Git | Static patterns. **Updated v0.16.10**: `.sdd-backup/` entry added for new projects; existing projects get the entry appended idempotently on `--upgrade` | **Very low** | Covered by Scenario 1 + Scenario 48 (idempotent append) |
| `template/package.json` | npm | Template has no direct deps (generated per-project) | **Very low** | N/A |
| `template/AGENTS.md` | sdd-devflow (human-facing) | **v0.16.10**: smart-diff protection added. Upgrade no longer unconditionally overwrites. User customizations preserved; `.new` backup contains fresh adapted version for manual merge. Doctor check #14 detects adapter-failure artifacts (`Backend patterns ()` empty parens). Root cause of the foodXPlorer v0.16.9 regression was a latent bug in `adaptAgentsMd` that produced broken output when scanner couldn't detect framework — now guarded | Was **high** (one broken field silently propagated to the whole file); now **low** (smart-diff + backup + doctor check + root-cause guard) | Already mitigated in v0.16.10 (Scenarios 43, 44, 46) |
| `template/.claude/agents/*.md` and `template/.gemini/agents/*.md` (template agents) | sdd-devflow (human-facing) | **v0.16.10**: smart-diff protection added, mirroring the standards smart-diff pattern. Per-file comparison against `adaptAgentContentString(rawTemplate, file, projectType)`. Custom agents (not in `TEMPLATE_AGENTS`) untouched on upgrade | Was **high** (wholesale delete-and-copy of the agents directory would erase user customizations); now **low** (smart-diff + backup) | Already mitigated in v0.16.10 (Scenarios 43–47) |
...
5. **No provenance tracking — cross-version pristine upgrades trigger false-positive preserve warnings (v0.16.10 accepted trade-off, v0.17.0 will fix)**: the v0.16.10 smart-diff compares the user's file against `adaptAgentContentString(newRawTemplate, file, projectType)`. On any future version bump that changes a template agent or `AGENTS.md`, a pristine file from the previous version will NOT match the new adapted target — the upgrade flags it as customized and preserves it. Users have to re-run `--upgrade --force --force-template --yes` (escape hatch added in v0.16.10) to accept the new template content, or manually `cp .sdd-backup/<ts>/<path>.new <path>` for each file. This is **NOT silent loss** — the upgrade prints a clear block explaining exactly what to do — but it IS friction. Codex cross-model review raised this as P1 in post-implementation; the finding was already raised as H-severity in the plan review and explicitly deferred to v0.17.0. Fix path: `.sdd-content-hashes.json` with SHA-256 of each adapted template file, stored at install/upgrade completion, consulted at next upgrade. Fall back to current conservative behavior when hashes are missing (first v0.17.0 upgrade from a pre-v0.17.0 project).
